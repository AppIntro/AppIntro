...
public abstract class AppIntro2 extends AppCompatActivity {
	...
	static enum TransformType {
		FLOW, DEPTH, ZOOM, SLIDE_OVER, FADE
	}
	...
	final protected void onCreate(Bundle savedInstanceState) {
		...
		/**
		 * ViewPager.setOnPageChangeListener is now deprecated. Use
		 * addOnPageChangeListener() instead of it.
		 */
		pager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
			...
			public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
				...
				if (transitionColors != null) {
					if (position < (pager.getAdapter().getCount() - 1) && position < (transitionColors.size() - 1)) {
						pager.setBackgroundColor((Integer) argbEvaluator.evaluate(positionOffset,
								transitionColors.get(position), transitionColors.get(position + 1)));
					} else {
			...
			public void onPageSelected(int position) {
				...
				// Allow the swipe to be re-enabled if a user swipes to a
				// previous slide. Restore
				// state of progress button depending on global progress button
				// setting
		...
		pager.setCurrentItem(savedCurrentItem); // required for triggering
												// onPageSelected for first page
	 ...
	 * amount of slides. In this case there could not be enough space to display
	 * all dots on smaller device screens.
	 */
	 ...
	 * for the {@link AppIntro2} instead of the default one.
	 *
	 * @param controller
	...
	/**
	 * Setting to disable forward swiping right on current page and allow
	 * swiping left. If a swipe left occurs, the lock state is reset and swiping
	 * is re-enabled. (one shot disable) This also hides/shows the Next and Done
	 * buttons accordingly.
	 *
	 * @param lockEnable
	 *            Set true to disable forward swiping. False to enable.
	 */
	 ...
	 * and Size of the color array must be equal to the number of slides added
	 * 
	 * @param colors
	 *            Set color values
	 */
